...
static int core_func(void *data) {
    ...
    ktime_t before,after,diff,exec,per;
    int i = 0;
    int subtask_index = 0;
    ...
    for(i = 0; i < sum; i++){         
        ...
        if(i == sum - 1){
           ...
           per = ktime_set(0, task_set[subtask[no].parent_task_no].task_period*MILLION_SEC);

           if(ktime_compare(diff,per) < 0){
             printk("Current round is finished! Starting next round....");
             diff = ktime_sub(per,diff);
           } 
