...
static void priority_computing(void){
    ...
    for( i = 0; i < SUBTASK_NUM; i++){
        printk(KERN_INFO "subtask[%d]: path[%d], priority[%d]",subtask_set[i].subtask_no, subtask_set[i].path,subtask_set[i].priority);
    }
}
...
static void sort_priority_in_task(void){
    ...
    for(i = 0; i< TASK_NUM;i++){
        for(j = 0; j < task_set[i].subtask_sum_in_task; j++){
	    printk(KERN_INFO "task[%d]:subtask[%d]: path[%d],priority[%d],prev_num[%d]",i,task_set[i].subtask_arr[j]->subtask_no,task_set[i].subtask_arr[j]->path,task_set[i].subtask_arr[j]->priority,task_set[i].subtask_arr[j]->prev_num);
        }
    }
}
...
static void high_task_scheduling(void){
    ...
    for(i = 0; i < TASK_NUM ; i++){
        ...
        if(task_set[i].high_flag == 1){
            ...
            earliest = start;
            while(finished < subtask_num && bound < 20){
...
static void low_task_scheduling(void){
    ...
    earliest = start;
    while(finished < low_subtask_count && bound < 20){
        bound++;
        //worst-fit
        earliest = start + (earliest + 1 - start) % core_num;
...
static void testsetting(void) {
    ...
    //initialize subtask_set;
    for(i = 0; i < SUBTASK_NUM;i++){
        subtask_set[i].parent_task_no = subtask_config2_tbl[i][Task_No];
        subtask_set[i].segment_no = subtask_config2_tbl[i][Seg_No];
        subtask_set[i].subtask_no = subtask_config2_tbl[i][Sub_No];
	subtask_set[i].relative_ddl = subtask_config2_tbl[i][Rela_Ddl];
        subtask_set[i].exec_time = subtask_config2_tbl[i][Exec_Time];
        subtask_set[i].priority = subtask_config2_tbl[i][Prio];
        subtask_set[i].core_no = subtask_config2_tbl[i][Core_No];
        subtask_set[i].release_time = subtask_config2_tbl[i][Release_Time];
        subtask_set[i].heavy_flag = subtask_config2_tbl[i][Heavy_Flag];
	subtask_set[i].loop_count = subtask_set[i].exec_time * LOOP_COUNT;
        sorted_subtask_arr[i] = i;
        printk(KERN_INFO "subtask[%d]:task_no[%d],segment_no[%d],subtask_no[%d],relative_ddl[%d],exec_time[%d],priority[%d],core_no[%d],release_time[%d],heavy_flag[%d]",i,subtask_set[i].parent_task_no,
	subtask_set[i].segment_no,subtask_set[i].subtask_no,subtask_set[i].relative_ddl,subtask_set[i].exec_time,subtask_set[i].priority,subtask_set[i].core_no,subtask_set[i].release_time,subtask_set[i].heavy_flag);
    }
	...
	task_set[i].task_no = i;
        ...
        task_set[i].critical_path = task_config[i][Cri_Path];
        task_set[i].util = task_config[i][Util];
        task_set[i].offset = task_config[i][Offset];
        task_set[i].subtask_sum_in_task = 0;
	    ...
	    task_set[i].seg_arr[j].segment_period = task_set[i].seg_arr[j].subtask_arr[0]->relative_ddl;
	    task_set[i].seg_arr[j].seg_heavy = task_set[i].seg_arr[j].subtask_arr[0]->heavy_flag;
...
static int simple_init (void) {
    ...
    //Scheduler Mode:
    if (sysfs_streq(mode_str, "scheduler")){
        //compute precursor number;
        prev_node_num_computing();
        //compute critical path;
        path_computing();
        //compute priority based on critical path;
	priority_computing();
        //sort subtas
