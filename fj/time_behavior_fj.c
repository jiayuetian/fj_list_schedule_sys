...
static void first_scheduling(void){
    int i, j, k = 0;
    int temp = 0;
    int x = 0;
    for( i=0; i<SUBTASK_NUM; i++){
        ...
        for ( j=0; j<CORE_NUM;j++){
            if(subtask_set[x].release_time <= core_set[temp].last_release){
                ...
                if((core_set[temp].last_release + subtask_set[x].exec_time) > (subtask_set[x].release_time + subtask_set[x].relative_ddl)){ 
                                  printk("!!!SUBTASK[%d] IS UNSCHEDUABLE!!!",x);
                    }
		    ...
		    //printk(KERN_INFO "core_set[%d].last_release[%d]    subtask_set[%d]:exec_time[%d] release_time[%d] relative_ddl[%d] ",
                ...
                subtask_set[x].core_no = temp;
                subtask_set[x].finish_time = core_set[temp].last_release;
                //printk(KERN_INFO "subtask[%d] is assigned to core[%d]",x,temp);
		temp = (temp + 1)%CORE_NUM;
                break;
            }
...
static void worst_scheduling(void){
    int i, j, k = 0;
    int temp = 0;
    int x = 0;
    ...
    for( i=0; i<SUBTASK_NUM; i++){
        ...
        temp = 0;
        for( j=0; j < CORE_NUM; j++){
            if((subtask_set[x].relative_ddl - core_set[j].last_release) > longest_interval){
                temp = j;
                longest_interval = (subtask_set[x].relative_ddl - core_set[j].last_release);
              }
        }
...
enum hrtimer_restart timer_start( struct hrtimer *timer ) {
    printk(KERN_INFO "in timer_start handler");
    int i,j = 0;
    if(run_num >= RECUR_TIME)  return HRTIMER_NORESTART;
    if(finished < FINISH_BOUND)
       {
        printk(KERN_INFO "Not finished![%d] Cannot start next round",finished);
        hrtimer_start(&start_timer,ktime_set(0, 1000*MILLION_SEC),HRTIMER_MODE_REL);
       }
    else{
         ...
         run_num++;
         printk(KERN_INFO "Current_finished:[%d] run_num[%d]",finished,run_num);
         hrtimer_start(&start_timer,ktime_set(0, 20*MILLION_SEC),HRTIMER_MODE_REL);
        }
            ...
            break;
               ...
               for( i=0; i < task_set[task].seg_arr[current_seg].subtask_num; i++ ){
        	    ...
        	    sched_setscheduler(task_set[task].thread_arr[core], SCHED_FIFO, &para);
                    ...
                    ktime_t diff = ktime_sub(ktime_get(),ktime_add(base, task_set[task].seg_arr[current_seg].subtask_arr[i]->finish_time*MILLION_SEC));
                    struct timespec time_interval = ktime_to_timespec(diff);
                    printk("subtask[%d]: diff:sec:%d, nsec:%d",task_set[task].seg_arr[current_seg].subtask_arr[i]->no, time_interval.tv_sec,time_interval.tv_nsec);
              ...
              // }
               ...
               if(ktime_compare(curr, next) < 0){
                  ...
                  if(current_seg == task_set[task].segment_num -1){
                     printk("PERIOD DEADLINE MISS");
                  }
                  else{printk("segment deadline MISS");}
...
static int simple_init (void){
    ...
    if (sysfs_streq(mode_str, "schedule")){
        ...
        if(sysfs_streq(assign_str, "first")){
	   first_scheduling();
        }

        if(sysfs_streq(assign_str, "worst")){
	   worst_scheduling();
        }
	...
	//    subtask_set[i].segment_no,subtask_set[i].subtask_no,subtask_set[i].exec_time,subtask_set[i].relative_ddl,subtask_set[i].priority,subtask_set[i].core_no,subtask_set[i].release_time,subtask_set[i].heavy_flag);
        ...
        start_timer.function = &timer_start;
        finished = FINISH_BOUND;
